...
- [x] Sticky navigator chrome reduced and anchor offsets recalculated (2026-06-16, follow-up): the desktop sticky bar carried a full `min-h-28` (112px) previous/next preview panel alongside the title/progress row, making the persistent chrome occupy a large share of the viewport (confirmed by user screenshot). Replaced the two large `ModuleLink` preview panels with the existing compact icon-only `CompactModuleLink` buttons (already used on mobile), collapsed the title/progress/prev-next into a single row, and thinned the progress bar and section-tab strip padding — cutting the sticky bar's height roughly in half. Anchor `scroll-mt` was previously a flat `scroll-mt-44` tuned for the old, much taller bar; it is now responsive (`scroll-mt-32 lg:scroll-mt-48`) since mobile has no sticky `LessonNavigator` (only the sticky `Header`) while desktop stacks the sticky header and the (now shorter) sticky navigator — this also fixes anchor-jump targets landing under/behind the chrome ("scroll past the chrome") on both breakpoints. Visual confirmation at exact pixel offsets still needs a browser, but the height math now tracks the actual rendered chrome far more closely than before.
